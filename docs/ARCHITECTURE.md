...
*   **Gestión de Feeds de Afiliados:** Procesamiento de fuentes complejas como Tradedoubler (Farma2Go). Se ha implementado de forma exitosa un **motor de historial de precios propio** (Price History Engine) operativo, que registra variaciones en la tabla `precios_historicos` de la base de datos y permite compensar la falta de precio base o MSRP consistente en los feeds de terceros para identificar chollos reales a largo plazo. El motor cuenta con protección contra imprecisiones de coma flotante (round(precio, 2)) para evitar registros duplicados fantasma y scripts de sanitización automáticos.
...
*   **Sistema de Analítica Full-Funnel (GA4):** Infraestructura de medición nativa (vía @next/third-parties) que traza todo el recorrido del usuario sin penalizar el rendimiento SSR: Top of Funnel (search_query, filter_applied), Middle of Funnel (view_item, product_interaction) y Bottom of Funnel (click_affiliate mediante wrappers asíncronos de cliente).
