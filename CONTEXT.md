...
| **Fix Scroll Offset Navbar** | Frontend | **COMPLETADO** | Se inyectó `scroll-mt-24` al ID del catálogo y `scroll-smooth` al `<html>` en el layout raíz para evitar el solapamiento con la Navbar fija y lograr una navegación por anclas elegante. |
| **Limpieza Visual Banner Ofertas** | Frontend | **COMPLETADO** | Eliminación completa del banner flotante "Filtro activo: Productos en oferta destacada" en `Catalog.tsx`, garantizando que la lista de resultados y el contador queden como elemento superior de la vista. |
| **Corrección Final Hover Top Ofertas** | Frontend | **COMPLETADO** | Botón inactivo refactorizado utilizando el selector `group` y directivas `group-hover:text-slate-900 group-hover:font-bold` para asegurar un comportamiento idéntico al resto de botones del menú principal. |
| **Unificación Cromática Top Ofertas** | Frontend | **COMPLETADO** | Botón inactivo fijado a `text-slate-700` opaco. Botón activo y barra de filtro en `Catalog.tsx` unificados bajo la misma paleta exacta (`bg-amber-50`, borde `amber-200`, texto `amber-900`) para coherencia total. |
| **Corrección Colores Top Ofertas** | Frontend | **COMPLETADO** | Corregidos problemas de legibilidad. Estado inactivo (`text-slate-700`), estado activo (`bg-amber-50/90 text-slate-900`), y el banner de `Catalog.tsx` rediseñado en tonos neutros (`bg-slate-50 text-slate-900`) asegurando un diseño premium. |
| **Rediseño Módulo Top Ofertas** | Frontend | **COMPLETADO** | Botón de la Navbar simplificado y condicionado por el fondo sin etiquetas tipográficas redundantes. Reemplazo del banner gigante rojo promocional en el catálogo por una barra de estado compacta y elegante en tonos ámbar. |
| **Erradicación Fondo Oscuro Desplegable** | Frontend | **COMPLETADO** | Popover de `SearchOmnibox.tsx` actualizado con `!bg-white shadow-2xl` y clases internas limpiadas (`!bg-slate-50`, `!text-slate-800`), eliminando permanentemente cualquier herencia indeseada del modo oscuro que afectaba la legibilidad. |
| **Rediseño Estado Vacío Omnibox** | Frontend | **COMPLETADO** | Estado sin resultados en `SearchOmnibox.tsx` actualizado a contenedor `bg-white` amplio (`p-6`), con SVG de búsqueda, título en alto contraste (`text-slate-900`) y subtexto de orientación. |
| **Corrección Crítica Omnibox (Input & Popover)** | Frontend | **COMPLETADO** | Reemplazo estructural exacto del input y el estado vacío de `SearchOmnibox.tsx`. Aplicación de `style={{color: '#0f172a'}}` inquebrantable para el texto, y reescritura total del popover flotante a fondo puramente blanco, erradicando el azul oscuro remanente. |
| **Corrección Crítica Botón Favoritos** | Frontend | **COMPLETADO** | Reemplazo estructural exacto del botón Favoritos en `Navbar.tsx` para garantizar texto oscuro (`text-slate-800`), fondo gris claro y hover funcional en negrita y corazón granate, según las estrictas indicaciones. |
| **Corrección Final 3 Fallos UI (Navbar & Omnibox)** | Frontend | **COMPLETADO** | Corregida la invisibilidad del cursor y texto del input en `SearchOmnibox.tsx` (`caret-slate-900 text-slate-900`). Desplegable ajustado a blanco limpio `bg-white` y botón Favoritos en `Navbar.tsx` rediseñado con estructura de botón propio y hover negrita sin conflictos de color. |
| **Correcciones Visibilidad CSS (Favoritos & Omnibox)** | Frontend | **COMPLETADO** | Corregida la invisibilidad del botón Favoritos aplicando alto contraste (`text-slate-800`). Corregido input Omnibox (`text-slate-900`) y ajustado desplegable a blanco limpio eliminando fondos oscuros desalineados. |
| **Corrección 4 Fallos Navbar & Omnibox** | Frontend | **COMPLETADO** | Botón Acceder idéntico en simetría a Salir (`bg-slate-100/80`), Favoritos sobrio estilo Prozis (`text-slate-700` sin fondos rosa), Omnibox en alto contraste con desplegable blanco sobrio (`bg-white rounded-2xl shadow-xl`), y eliminación completa de la píldora `Ctrl+K`. |
| **Redecoración SaaS Navbar & SearchOmnibox** | Frontend | **COMPLETADO** | Estética SaaS de alto nivel en `Navbar.tsx` y `SearchOmnibox.tsx`. Cero emojis (SVG llama en Top Ofertas), atajo `⌘K`/`Ctrl+K` con listener global, botones neutros elegantes para "Acceder", "Favoritos" rosa suave e iluminación glassmorphism. |
| **Unificación Buscador Omnibox & Hero** | Frontend | **COMPLETADO** | Eliminado el `<input>` secundario redundante del Hero en `Catalog.tsx`, conservando intacta la tira de logos de marcas clicables. `SearchOmnibox.tsx` en `Navbar.tsx` es el único punto de búsqueda global. Compresión vertical optimizada para visibilidad 'above the fold'. |
| **Buscador Omnibox Global (Live)** | Frontend | **COMPLETADO** | Componente `SearchOmnibox.tsx` integrado en `Navbar.tsx` con debounce de 250ms, sugerencias live de hasta 4 ítems con thumbnails (44x44px), precios en alto contraste, footer 'Ver todos', soporte para tecla `Escape` y cierre en click-outside. |
| **Alto Contraste Marca/Título & SEO** | Frontend | **COMPLETADO** | Colores fijados a `text-slate-900` para marca y título en `ProductCard.tsx`. Legibilidad 100% inmediata sobre fondo blanco con hover azul en título. |
