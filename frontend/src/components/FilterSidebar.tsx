...
export default function FilterSidebar({
  ...
  // Contenido interno compartido de los filtros (idéntico para ambos)
  const filterContent = (
    <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm flex flex-col gap-3.5 w-full text-left">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        >
          ...
          <svg
            className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-200 ${openSections.category !== false ? "rotate-180" : ""
              }`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
      ...
      <div className="flex flex-col gap-1.5">
        ...
        >
          ...
          <svg
            className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-200 ${openSections.subfilters !== false ? "rotate-180" : ""
              }`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
      ...
      <div className="flex flex-col gap-2">
        ...
        >
          ...
          <svg
            className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-200 ${openSections.brand !== false ? "rotate-180" : ""
              }`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
              ...
              {brands
                ...
                .map((brand) => {
                  ...
                  return (
                    <label
                      key={brand}
                      className={`flex items-center justify-between px-1.5 py-0.5 rounded text-[11px] font-medium cursor-pointer transition-colors select-none ${isChecked
      ...
      {/* Botón de cerrar resultados en móvil */}
      {isMobileFilterOpen && (
        <button
  ...
  return (
    <>
      {/* 1. ESCRITORIO: Intacto tal como estaba originalmente (sticky sidebar) */}
      <aside className="hidden md:block sticky top-20 w-[280px] flex-shrink-0 max-h-[calc(100vh-6rem)] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-200 text-left">
        {filterContent}
      </aside>

      {/* 2. MÓVIL: Portal flotante independiente que cubre toda la pantalla desde el píxel 0 */}
      {mounted && isMobileFilterOpen && createPortal(
        <div className="fixed inset-0 z-[999999] bg-white flex flex-col h-[100dvh] w-full overflow-hidden md:hidden">
          {/* Cabecera fija superior con el título y el botón de cierre */}
          <div className="min-h-[60px] border-b border-slate-200 px-4 py-3 flex justify-between items-center bg-white shrink-0 shadow-xs">
            <h2 className="text-xl font-black text-slate-900">Filtrar Catálogo</h2>
            <button
              onClick={() => setIsMobileFilterOpen(false)}
              className="p-1.5 bg-slate-100 rounded-full text-slate-600 hover:bg-slate-200 transition-colors cursor-pointer"
            >
          ...
          {/* Contenedor con scroll interno que empieza estrictamente arriba con la Categoría Principal */}
