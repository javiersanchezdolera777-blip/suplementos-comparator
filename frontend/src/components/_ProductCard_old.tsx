...
interface Product {
  ...
  // El backend ahora puede enviar m├║ltiples sabores por producto
...
const sanitizeDescription = (text?: string | null): string => {
  ...
  // 1. Eliminar preguntas gen├®ricas de relleno SEO al inicio del texto (ej: "┬┐por qu├® debemos utilizar...?")
  cleaned = cleaned.replace(/^┬┐por qu├® [^?]+\?\s*/i, '');

  // 2. Corregir puntos pegados a palabras sin espacio (ej: "apat├¡a.es frecuente" -> "apat├¡a. es frecuente")
  cleaned = cleaned.replace(/\.([a-zA-Z├í├®├¡├│├║├▒├ü├ë├ì├ô├Ü├æ])/g, '. $1');

  // 3. Capitalizar la primera letra despu├®s de un punto y seguido (ej: ". los m├ís" -> ". Los m├ís")
  cleaned = cleaned.replace(/\.\s+([a-z├▒├í├®├¡├│├║])/g, (_, letter) => `. ${letter.toUpperCase()}`);

  // 4. Capitalizar la primera letra alfab├®tica del texto (incluso si empieza por signos como ┬┐, ", ()
  cleaned = cleaned.replace(/^([^a-zA-Z├í├®├¡├│├║├▒├ü├ë├ì├ô├Ü├æ]*)([a-z├▒├í├®├¡├│├║])/, (_, prefix, letter) => prefix + letter.toUpperCase());
...
const formatTitle = (nombre: string, marca?: string, presentacion?: string) => {
  ...
  if (presentacion) {
    ...
    const regexPres = new RegExp(`[\\s\\-,|]*${escapedPres}(?![a-zA-Z├í├®├¡├│├║├ü├ë├ì├ô├Ü])`, 'gi');
...
export default function ProductCard({ product }: { product: Product }) {
  ...
  // L├│gica estricta para extraer tienda vendedora real (NUNCA usar marca como fallback)
  const rawStore =
    ...
    <>
      ...
      >
        ...
        <div className="relative h-32 md:h-48 p-2 md:p-4 flex items-center justify-center bg-slate-50 overflow-hidden">
          ...
          {/* Bot├│n VS (Comparar) flotante debajo del coraz├│n */}
          <div className="absolute top-14 right-3 z-20 group/vs cursor-pointer active:scale-125 transition-transform duration-200" onClick={handleCompare}>
            <div className={`p-2 rounded-full border transition-all duration-200 shadow-sm flex items-center justify-center w-[34px] h-[34px] ${isCompared
              ...
              }`} title={isCompared ? "Ya en la comparativa" : "A├▒adir a comparativa"}>
        ...
        {/* Zona Inferior: Informaci├│n del producto (Limpia y Sobria con Alto Contraste) */}
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mt-auto pt-3 md:pt-4 gap-2">
            <div className="flex items-center flex-wrap gap-1.5">
              <span className="flex items-baseline gap-1 whitespace-nowrap text-lg md:text-2xl font-extrabold text-slate-900 tracking-tight">
                ...
                <span className="text-base md:text-xl font-bold text-slate-700">Ôé¼</span>
              </span>
              {hasOffer && (
                <span className="flex items-baseline gap-1 whitespace-nowrap text-xs md:text-sm font-semibold text-slate-400 line-through ml-1">
                  ...
                  <span className="text-[10px] md:text-xs">Ôé¼</span>
                </span>
              )}
            </div>
      ...
      {mounted && isModalOpen && createPortal(
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            {/* Micro-Toast Notificaci├│n Favoritos */}
            {toastMsg && (
            ...
            {/* Columna Derecha: Informaci├│n y Scroll Exclusivo para Textos */}
            <div className="w-full md:w-1/2 flex flex-col h-full bg-white p-6 md:p-8 relative overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar my-2">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-100 text-sm w-full">
                  <div className="flex flex-col"><span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Categor├¡a</span><span className="text-slate-700 font-medium">{product.category?.name || '-'}</span></div>
                  ...
                  <div className="flex flex-col"><span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Tama├▒o</span><span className="text-slate-700 font-medium">{product.presentacion || '-'}</span></div>
                  ...
                  {product.is_vegan && <div className="flex flex-col"><span className="text-[10px] text-emerald-600 uppercase font-bold tracking-wider">Diet├®tico</span><span className="text-emerald-700 font-medium">100% Vegano</span></div>}
                  {product.protein_type && <div className="flex flex-col"><span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Prote├¡na</span><span className="text-slate-700 font-medium">{product.protein_type}</span></div>}
                  {(product.protein_percentage || product.porcentaje_proteina) && (
                    <div className="flex flex-col">
                      <span className="text-[10px] text-blue-600 uppercase font-bold tracking-wider">% Prote├¡na</span>
              ...
              <div className="pt-4 border-t border-slate-100 mt-auto">
                ...
                <div className="mt-4 text-center">
                  ...
                  >
                    Ver ficha t├®cnica completa &rarr;
                  </Link>
                </div>
              </div>
            </div>
