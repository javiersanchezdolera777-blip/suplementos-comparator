...
class Perfil(Base):
    ...
    objetivo_etapa = Column(
        String, default="Mantenimiento"
    )  # Puede ser: "Volumen", "Definición", "Mantenimiento"

    # Relaciones
    usuario = relationship("Usuario", back_populates="perfil")
    stacks = relationship(
        "Stack", back_populates="creador", cascade="all, delete-orphan"
    )
    resenas = relationship(
        "ResenaSabor", back_populates="perfil", cascade="all, delete-orphan"
    )
...
class Stack(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    perfil_id = Column(
        Integer, ForeignKey("perfiles.id", ondelete="CASCADE"), nullable=False
    )
    ...
    )  # Por si quieren guardar un stack privado
