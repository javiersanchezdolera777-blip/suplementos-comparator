...
def generar_slug(nombre: str) -> str:
    texto = unicodedata.normalize('NFKD', nombre).encode(
...
def extraer_porcentaje_proteina(texto: str):
    ...
    m2 = re.search(
        r'(\d{2}(?:[.,]\d+)?)\s*g\s*(?:de\s*)?prote[íi]na[^\d]{1,30}(\d{2,3}(?:[.,]\d+)?)\s*g', texto)
    if m2:
    ...
    m3 = re.search(
        r'(\d{2}(?:[.,]\d+)?)\s*%\s*(?:de\s*)?(?:prote[íi]na|pureza|wpc|wpi|cfm|whey|aislado)', texto)
    if m3:
        return round(float(m3.group(1).replace(',', '.')))

    m4 = re.search(
        r'(?:wpc|wpi|cfm|whey|prote[íi]na|pureza|concentración|proteico)[^\d]{0,20}(\d{2}(?:[.,]\d+)?)\s*%', texto)
    if m4:
...
def calcular_metricas_precio(nombre: str, descripcion: str, precio: float):
    metricas = {"peso_gramos": None, "precio_por_kg": None,
                "unidades": None, "precio_por_unidad": None}
    match_unidades = re.search(
        r'(\d+)\s*(cap|caps|cápsulas|capsulas|comprimidos|pastillas|perlas|viales|uds|unidades|tablets|tabletas)\b', nombre)
    es_pastilla = False
...
def inyectar_en_bd():
    ...
    try:
        ...
        marca_hsn = db.query(models.Marca).filter_by(
            nombre=nombre_marca).first()
        if not marca_hsn:
            try:
        ...
        productos_bd = {p.slug: p for p in db.query(
            models.Producto).filter_by(tienda="HSN").all()}
        print(f"✨ {len(productos_bd)} productos en memoria. Iniciando ingesta...")

        mapa_categorias = {}
        for cat in CategoriaEnum:
            cat_db = db.query(models.Categoria).filter_by(
                nombre=cat.value).first()
            if not cat_db:
                try:
        ...
        while pagina <= max_paginas:
            ...
            try:
                try:
                    ...
                    print(
                        f"   ⚠️ No se pudo cargar la página {url_pagina}: {net_err}")
                    break
                ...
                enlaces_pagina = [a.get('href') for a in soup_cat.select(
                    '.product-item-link') if a.get('href')]
                ...
                for url_prod in enlaces_pagina:
                    ...
                    try:
                        try:
                            res_prod = get_with_backoff(
                                session, url_prod, timeout=20)
                        except Exception as net_err:
                            print(
                        ...
                        try:
                            ...
                            bloques = re.findall(
                                r'<script[^>]+type=["\']application/ld\+json["\'][^>]*>(.*?)</script>', res_prod.text, flags=re.S | re.I)
                        ...
                        nombre = datos_producto.get(
                            'name') if datos_producto else None
                        if not nombre:
                            title_tags = soup_prod.find_all(
                                'meta', property='og:title')
                            if title_tags:
                                nombre = title_tags[-1].get('content',
                                                            '').split('|')[0].strip()
                            else:
                                title_h1 = soup_prod.find(
                                    'h1', class_='page-title')
                                nombre = title_h1.get_text(
                                    strip=True) if title_h1 else None
                        ...
                        if not imagen:
                            img_tag = soup_prod.find(
                                'meta', property='og:image')
                            imagen = img_tag.get(
                                'content', '') if img_tag else ''

                        desc_cruda = datos_producto.get('description')
                        if not desc_cruda:
                            desc_tag = soup_prod.find(
                        ...
                        try:
                            p_sale = float(sale_tag['content']) if sale_tag and sale_tag.get(
                                'content') else 0.0
                            p_base = float(base_tag['content']) if base_tag and base_tag.get(
                                'content') else 0.0
                        ...
                        if 'soup_prod' in locals():
                            try:
                                titulo_pagina = soup_prod.find(
                                    'title').text if soup_prod.find('title') else ''
                        ...
                        textos_sabores = [op.get("option-label", "")
                                          for op in opciones_sabor]
                        ...
                        try:
                            ...
                            precio_ant_norm = float(precio_anterior) if precio_anterior not in (
                                None, '', 0) else None
                            slug_norm = generar_slug(nombre_norm)
                            peso_norm = int(metricas.get('peso_gramos')) if metricas.get(
                                'peso_gramos') else None
                            preciokg_norm = float(metricas.get('precio_por_kg')) if metricas.get(
                                'precio_por_kg') else None
                            porcentaje_proteina = etiquetas.get(
                                'porcentaje_proteina')
                            try:
                                porcentaje_proteina = int(
                                    porcentaje_proteina) if porcentaje_proteina is not None else None
                            except Exception:
                            ...
                            if slug_norm in productos_bd:
                                ...
                                p_existente.es_vegano = bool(
                                    etiquetas.get("es_vegano"))
                                ...
                                nuevo_prod = models.Producto(
                                    ...
                                    sin_lactosa=bool(
                                        etiquetas.get("sin_lactosa")),
                                    sello_calidad=etiquetas.get(
                                        "sello_calidad"),
                                    tipo_proteina=etiquetas.get(
                                        "tipo_proteina"),
                                    porcentaje_proteina=porcentaje_proteina,
                                    tipo_creatina=etiquetas.get(
                                        "tipo_creatina"),
                                    perfil_aminoacidos=etiquetas.get(
                                        "perfil_aminoacidos"),
                                    tipo_vitamina=etiquetas.get(
                                        "tipo_vitamina"),
    ...
    if productos_nuevos:
        try:
            ...
            db.rollback()
            print(
                f"⚠️ Aviso: No se pudieron guardar los últimos {len(productos_nuevos)} productos. Error: {e.__class__.__name__}: {e}")
            traceback.print_exc()
